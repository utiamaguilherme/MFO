...
Example all_zero_test: all_zero (bits B1 B0 B0 B0) = false.
  Proof. simpl. reflexivity. Qed.
...
Definition invert_nybble(n:nybble) : nybble :=  
  match n with
  | bits b1 b2 b3 b4 => bits (inv b1) (inv b2) (inv b3) (inv b4)
  end.
  
Compute (invert_nybble (bits B1 B0 B0 B1)).

Theorem inv_involutive : forall (b:bit), inv(inv b) = b.
  Proof. intro b. destruct b.
  - simpl. reflexivity.
  - simpl. reflexivity.
Qed.

Theorem inv_nybble_involutive : forall(nb : nybble),
  invert_nybble (invert_nybble nb) = nb.
  Proof. intro nb. destruct nb as [b1 b2 b3 b4].
  - simpl. rewrite inv_involutive. rewrite inv_involutive. rewrite inv_involutive. rewrite inv_involutive.
  reflexivity.
 Qed.
...
Proof. simpl. reflexivity. Qed.
Example test_factorial2:          (factorial 5) = (mult 5 24).
Proof. simpl. reflexivity. Qed.
(** [] *)
